...
<body>
    ...
    <section class="notice">
        <div class="inner">
            ...
            <?php
            $pSql="SELECT * FROM counsel WHERE num < '$num' ORDER BY num DESC LIMIT 1";
            $pRes=mysqli_query($conn, $pSql);
            $pRow=mysqli_fetch_array($pRes);
            $prev=$pRow['num'];
            
            $nSql="SELECT * FROM counsel WHERE num > '$num' ORDER BY num ASC LIMIT 1";
            $nRes=mysqli_query($conn, $nSql);
            $nRow=mysqli_fetch_array($nRes);
            $next=$nRow['num'];
            ?>
            <div class="detail-bottom">
                <a href="counsel_request_detail.php?num=<?php echo $prev?>" style="<?php echo ($prev) ? "visibility:hidden" : "visibility:hidden"?>">
                    <img src="../img/notice-prev-arrow.png" alt="">
                    <p>이전 글 보기</p>
                </a>
                <a href="inquiry.php"><span>목록</span></a>
                <a href="counsel_request_detail.php?num=<?php echo $next?>" style="<?php echo ($next) ? "visibility:hidden" : "visibility:hidden"?>">
                    <p>다음 글 보기</p>
