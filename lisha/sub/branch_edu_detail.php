    ...
    <section class="schedule">
        <div class="inner">
            ...
            <div class="detail_bottom">
                <?php
                $pSql="SELECT * FROM schedule WHERE category='교육' AND `local`='지부' AND num < '$num' ORDER BY num DESC LIMIT 1";
                $pRes=mysqli_query($conn, $pSql);
                $pRow=mysqli_fetch_array($pRes);
                $prev=$pRow['num'];
                
                $nSql="SELECT * FROM schedule WHERE category='교육' AND `local`='지부' AND num > '$num' ORDER BY num ASC LIMIT 1";
                $nRes=mysqli_query($conn, $nSql);
                $nRow=mysqli_fetch_array($nRes);
                $next=$nRow['num'];
                ?>
                <a href="<?php echo ($prev) ? "branch_edu_detail.php?num=".$prev : "javascript:void(0)"?>" style="<?php echo ($prev) ? "" : "visibility:hidden"?>"><img src="../img/list-prev.png" alt=""></a>
                <a href="head_edu.php">리스트</a>
                <a href="<?php echo ($next) ? "branch_edu_detail.php?num=".$next : "javascript:void(0)"?>" style="<?php echo ($next) ? "" : "visibility:hidden"?>"><img src="../img/list-next.png" alt=""></a>
            </div>
        </div>
    </section>
