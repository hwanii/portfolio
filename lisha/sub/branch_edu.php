...
<body>
    ...
    <section class="sub_banner">
        <div class="txt_box">
            ...
            <span>HOME&nbsp;&nbsp;>&nbsp;&nbsp;교육 일정&nbsp;&nbsp;>&nbsp;&nbsp;지부 교육일정</span>
        </div>
    </section>
    <div class="sub_category">
        <a href="../sub/head_edu.php">본사 교육일정</a>
        <a href="../sub/branch_edu.php" class="on">지부 교육일정</a>
    ...
    if($local){
        $l=" AND local_name='$local'";
    }?>
    <section class="schedule">
        <div class="inner">
            <div class="title">
                <h2>지부 교육일정</h2>
                <p>ILPA BRANCH SCHEDULE</p>
                <select onchange="change(this.value)">
                    <option value="">지부이름 선택</option>
                    <option value="제주" <?php echo ($local=="제주") ? "selected" : ""?>>제주 교육 지부</option>
                    <option value="동해" <?php echo ($local=="동해") ? "selected" : ""?>>동해 교육 지부</option>
                    <option value="광주" <?php echo ($local=="광주") ? "selected" : ""?>>광주 교육 지부</option>
                    <option value="위례" <?php echo ($local=="위례") ? "selected" : ""?>>위례 교육 지부</option>
                    <option value="김포" <?php echo ($local=="김포") ? "selected" : ""?>>김포 교육 지부</option>
                </select>
            </div>
            <div class="contents">
                <?php
                ...
                $pSql="SELECT COUNT(*) AS cnt FROM schedule WHERE `local`='지부'{$l}";
                ...
                $sql="SELECT * FROM schedule WHERE `local`='지부'{$l} ORDER BY num DESC LIMIT $limit, $list";
...
<script>
    function change(t){
        location.href="branch_edu.php?local="+t;
    }
</script>
</html>
