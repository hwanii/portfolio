...
<body>
    ...
    <?php
    $bSql="SELECT * FROM banner WHERE `type`='IR'";
    ...
    <section class="fix_des">
        ...
        <p>기업 성장 전 단계에 아우르는 유망 신기술사업자를 집중 발굴하여<br> 투자시장 활성화에 기여합니다.</p>
    </section>
    <section class="announcement detail">
        <div class="inner">
            ...
            <div class="an_table">
                ...
                $hSql="UPDATE ir SET hit=hit+1 WHERE num='$num'";
                $hRes=mysqli_query($conn, $hSql);

                $sql="SELECT * FROM ir WHERE num='$num'";
                ...
                ?>
                <a href="<?php echo ($file) ? "../file/ir/{$file}" : "javascript:void(0);"?>" class="contents" style="border-bottom: 1px solid #b0d1e8; background-color: #f2f2f2;" <?php echo ($file) ? "download" : ""?>>
                ...
                <div class="an_ta_mob">
                    ...
                    <div class="bbot">
                        <div class="ccontents">
                            ...
                            <a id="down" href="../file/ir/<?php echo $file?>" download style="dispaly:none"></a>
                        </div>
                        <div class="ccontents">
                            <p>작성일자</p>
            ...
            <div class="detail_bottom">
                <?php
                $pSql="SELECT * FROM ir WHERE num < '$num' ORDER BY num DESC LIMIT 1";
                $pRes=mysqli_query($conn, $pSql);
                $pRow=mysqli_fetch_array($pRes);
                $prev=$pRow['num'];
                
                $nSql="SELECT * FROM ir WHERE num > '$num' ORDER BY num ASC LIMIT 1";
                $nRes=mysqli_query($conn, $nSql);
                $nRow=mysqli_fetch_array($nRes);
                $next=$nRow['num'];
                ?>
                <a href="ir_detail.php?no=<?php echo $no-1;?>&num=<?php echo $prev?>" style="<?php echo ($prev=="") ? "visibility:hidden" : ""?>">
                    <img src="../img/detail-prev.png">
                    <p>이전글 보기</p>
                </a>
                <a href="ir.php" class="center">
                    <div>
                        <span></span>
                        <span></span>
                        <span></span>
                ...
                <a href="ir_detail.php?no=<?php echo $no+1;?>&num=<?php echo $next?>" style="<?php echo ($next=="") ? "visibility:hidden" : ""?>">
                    <p>다음글 보기</p>
